...
function draw(){
  ...
  if(worldId==='stacks' && wd.typewriter){
    const tw = wd.typewriter, twx = tw.tx*TILE+20-cam.x, twy = tw.ty*TILE+20-cam.y;
    drawSprite(SPR.stamper, twx, twy, 34);
    const gq = qstate.ghostwriter;
    if(gq && gq.status==='active' && gq.stage===2){
      ctx.font='bold 16px monospace'; ctx.fillStyle='#c58cff'; ctx.textAlign='center';
      ctx.fillText('!', twx, twy-26 + Math.sin(gameTime*4)*2);
    }
    nearLbl(tw.tx*TILE+20, tw.ty*TILE+20, '[E] the Night Typewriter');
  }
